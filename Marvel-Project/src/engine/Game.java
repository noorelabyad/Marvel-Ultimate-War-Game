...
public class Game {
	...
	public static boolean isBadAbility(Ability a)
	{
	...
	public void attack(Direction d)throws NotEnoughResourcesException, ChampionDisarmedException
	{
		...
		{
			case UP : 
				for (int i= x+1; i<5 && i<=(x+r)&& !hasAttacked; i++)
				{
					if (board[i][y] instanceof Cover)
					{
						Cover c2 = (Cover) board[i][y];
						c2.setCurrentHP(c2.getCurrentHP()-c.getAttackDamage());
						terminate(c2);
						hasAttacked=true;
					}
					else
					{
						if(board[i][y] instanceof Champion)
						{
							Champion c2 = (Champion) board[i][y];
							if (!areSameTeam(c,c2))
							{
								if(isFindEffect(c2,"Shield"))
								{
									Effect e = findEffect(c2,"Shield");
									c2.getAppliedEffects().remove(findEffect(c2,"Shield"));
									e.remove(c2);
								}
								else
								{
									int random =(int) (Math.random()*2);
									boolean dodge = isFindEffect(c2,"Dodge");
									if ((dodge && (random ==0))|| !dodge)
									{
										championTypeHelper(c,c2);
										hasAttacked=true;
										terminate(c2);
									}
			...
			case DOWN : 
				for (int i= x-1; i>=0 && i>=(x-r)&& !hasAttacked; i--)
				{
					if (board[i][y] instanceof Cover)
					{
						Cover c2 = (Cover) board[i][y];
			...
			case RIGHT : 
				for (int i= y+1; i<5 && i<=(y+r)&& !hasAttacked; i++)
				{
					if (board[x][i] instanceof Cover)
					{
						Cover c2 = (Cover) board[x][i];
						c2.setCurrentHP(c2.getCurrentHP()-c.getAttackDamage());
						terminate(c2);
						hasAttacked=true;
					}
					...
					{
						if(board[x][i] instanceof Champion)
						{
							Champion c2 = (Champion) board[x][i];
							if (!areSameTeam(c,c2))
							{
								if(isFindEffect(c2,"Shield"))
								{
			...
			case LEFT : 
				for (int i= y-1; i>=0 && i>=(y-r)&& !hasAttacked; i--)
				{
					if (board[x][i] instanceof Cover)
					{
						Cover c2 = (Cover) board[x][i];
	...
	public void championTypeHelper (Champion c, Champion c2)
	{
		if(c instanceof Hero)
		{
			if(c2 instanceof Villain || c2 instanceof AntiHero )
				c2.setCurrentHP( (c2.getCurrentHP() - (int)(c.getAttackDamage() * 1.5))); 
						else
							c2.setCurrentHP(c2.getCurrentHP() - c.getAttackDamage());
		}
		else
		{
			...
			{
				if(c2 instanceof Hero || c2 instanceof AntiHero)
					c2.setCurrentHP( (c2.getCurrentHP() - (int)(c.getAttackDamage() * 1.5))); 
				else
					c2.setCurrentHP(c2.getCurrentHP() - c.getAttackDamage());
			}
	...
	public void terminate(Damageable c)
	{
		if (c.getCurrentHP()==0)
		{
			...
			if (c instanceof Champion)
			{
				...
				if (isPresent((Champion)c, firstPlayer.getTeam()))
	...
	public static  boolean isFindEffect (Champion c, String name)
	{
	...
	public boolean areSameTeam ( Champion c, Champion c2)
	{
		return  ( (isPresent(c, firstPlayer.getTeam()) && isPresent(c2, firstPlayer.getTeam()) )
				|| (isPresent(c, secondPlayer.getTeam()) && isPresent(c2, secondPlayer.getTeam()) ));
	}
	public static boolean isPresent(Champion c , ArrayList <Champion> c2)
	{
		for (int i=0; i< c2.size(); i++)
		{
			if (c2.get(i).equals(c))
				return true;
		}
		return false;
	}
	...
	public void castAbility(Ability a)
		...
		{	Champion c = getCurrentChampion();
			if (!isBadAbility(a))
			{
				if (isPresent(c, firstPlayer.getTeam()))
				{
					for(int i =0; i<firstPlayer.getTeam().size(); i++)
					{
						if(withinRange(c, firstPlayer.getTeam().get(i), a))
				...
				else
				{
					...
					{
						if(withinRange(c, secondPlayer.getTeam().get(i), a))
			...
			else
			{
				if (a instanceof DamagingAbility)
				{
					if (isPresent(c, firstPlayer.getTeam()))
					{
						for(int i =0; i<secondPlayer.getTeam().size(); i++)
						{
							Champion c3 = secondPlayer.getTeam().get(i);
							if(isFindEffect(c3, "Shield")&& (withinRange(c, c3, a)))
							{
								Effect e = findEffect(c3,"Shield");
								c3.getAppliedEffects().remove(findEffect(c3,"Shield"));
								e.remove(c3);
							}
							else
								if(withinRange(c, secondPlayer.getTeam().get(i), a))
					...
					else
					{
						for(int i =0; i<firstPlayer.getTeam().size(); i++)
						{
							Champion c3 = firstPlayer.getTeam().get(i);
							if(isFindEffect(c3, "Shield") && (withinRange(c, c3, a)))
							{
								Effect e = findEffect(c3,"Shield");
								c3.getAppliedEffects().remove(findEffect(c3,"Shield"));
	...
	public boolean withinRange(Champion c, Champion c2, Ability a)
	{
		int distance = Math.abs(c.getLocation().x - c2.getLocation().x) + Math.abs(c.getLocation().y - c2.getLocation().y);
		return (a.getCastRange() >=distance);
	}
	...
	private void validateCastAbility(Ability a) throws NotEnoughResourcesException, AbilityUseException {
		if (getCurrentChampion().getMana() < a.getManaCost())
			...
			throw new AbilityUseException("You cannot cast an ability while being silenced!");
		else if (a.getCurrentCooldown() > 0)
			throw new AbilityUseException("You cannot use an ability while it is in cooldown!");
	}
	...
	private Effect  findEffect (Champion c, String s)
	{
		for (int i=0; i<c.getAppliedEffects().size();i++)
		{
			if((c.getAppliedEffects().get(i).getName()).equals(s))
				return c.getAppliedEffects().get(i);
		}
		return null;
	}
